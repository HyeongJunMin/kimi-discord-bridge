...
- **cmux-first wire fallback** — 기본 경로는 계속 `Discord → bridge → cmux → kimi-cli` 입니다. cmux `send/read`가 실패하고 Kimi session id가 저장돼 있으면 bridge가 `kimi --wire` SDK 세션으로 전환해 같은 메시지를 처리합니다. 잠금해제 후 restore worker가 workspace/surface 상태를 확인하고 새 cmux surface를 만든 뒤 `kimi --session <session_id>`를 실행해 cmux 경로로 되돌립니다. cmux가 `not_found` 계열 불일치 오류를 내면 circuit breaker가 restore 생성을 잠시 멈춰 surface 폭증을 막습니다.
