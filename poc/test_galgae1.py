#!/usr/bin/env python3
"""PoC — 갈래 1 (PreToolUse hook): block WriteFile via hook."""
...
async def main():
    ...
    ws = await create_workspace(name=f"poc-galgae1-{int(time.time())}",
                                 cwd=str(Path.cwd()))
    ...
    try:
        ...
        # 5. Send prompt FIRST (wire.jsonl is created when a turn starts)
        prompt = "Create a file named poc_test.txt with content 'hello from hook poc'.\n"
        log.info("sending prompt: %r", prompt)
        await surface_send_text(surf_id, prompt)

        # 6. Now find wire.jsonl
        ...
        if HOOK_LOG.exists():
            ...
            log.info("hook log:\n%s", log_text)
            if "WriteFile" in log_text:
                log.info("✅ Hook FIRED for WriteFile")
            else:
                log.warning("⚠️ Hook did NOT fire for WriteFile")
        else:
            log.warning("⚠️ No hook log found — hook may not have fired at all")

        # 10. Check wire.jsonl for block evidence
        ...
        log.info("tool_calls captured: %d", len(tool_calls))
        log.info("tool_results captured: %d", len(tool_results))
        ...
        if tool_calls and not tool_results:
            log.info("✅ PoC SUCCESS: ToolCall seen but NO ToolResult → hook blocked it")
        elif tool_results:
            for tr in tool_results:
                rv = tr.get("payload", {}).get("return_value", {})
                if rv.get("is_error"):
                    log.info("✅ PoC SUCCESS: ToolResult with is_error=true → hook blocked it")
                else:
                    log.info("⚠️ ToolResult without error → hook may have allowed it")
        else:
            log.info("⚠️ No tool activity observed")
